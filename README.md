...
**Status: color, perceptual quality, and CLIP semantic embeddings are
built and documented (below). SigLIP and geometric composition are real,
already-run parts of the project but not yet ported to this public repo.**
...
public image CDN. All eight download-capable scripts (everything except
`06`-`09`, which read `05`'s cache) share one poster cache
...
data/
  ...
  sample_output/   real, already-computed metrics for those same posters,
                    plus a real 99-poster clip_embeddings.npz generated for
                    this repo (see docs/RESULTS.md for what's verified to
                    reproduce exactly vs. what isn't, and why)
docs/
